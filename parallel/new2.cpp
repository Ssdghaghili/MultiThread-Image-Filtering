...
int main(int argc, char* argv[])
{
    ...
    parallelProcess(img_color, img_horizontal_mirror, horizontalMirror, numThreads);

    // Vertical Mirror
    Mat img_vertical_mirror = img_horizontal_mirror.clone();
    parallelProcess(img_horizontal_mirror, img_vertical_mirror, verticalMirror, numThreads);

    // Sepia Filter
    Mat img_sepia = img_vertical_mirror.clone();
    parallelProcess(img_vertical_mirror, img_sepia, sepiaFilter, numThreads);

    // Box Filter
    Mat img_box = img_gray.clone();
    parallelProcess(img_gray, img_box, boxFilter, numThreads);

    // Convert to Black and White
    Mat img_bw = img_box.clone();
    parallelProcess(img_box, img_bw, grayToBW, numThreads);

    // Laplacian Edge Detection
    Mat img_edge = img_bw.clone();
    parallelProcess(img_bw, img_edge, LaplacianEdgeDetection, numThreads);
