...
num_runs=5
total_time_serial=0
serial_input="../serial/ut.bmp"
parallel_input="../parallel/ut.bmp"
...
for ((i=1; i<=$num_runs; i++)); do
    ...
    runtime=$( { time -p ./ImageFilters.out "$serial_input" > /dev/null 2>&1; } 2>&1 | grep real | awk '{print $2}' )
...
for ((i=1; i<=$num_runs; i++)); do
    # Use time command to measure the execution time
    runtime=$( { time -p ./ImageFilters.out "$parallel_input" > /dev/null 2>&1; } 2>&1 | grep real | awk '{print $2}' )
